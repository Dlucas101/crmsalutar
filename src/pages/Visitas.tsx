...
import { CalendarDays, Plus, Check, Download, Clock, User } from "lucide-react";
...
// Brazilian national holidays (fixed + calculated)
...
export default function Visitas() {
  ...
  const getVisitsForDay = (day: Date) =>
    visits.filter((v) => isSameDay(new Date(v.visit_date), day));
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setForm({ titulo: "", descricao: "", visit_date: "", visit_time: "08:00", member_id: "", cor: MEMBER_COLORS[0] });
    setOpenNew(false);
    fetchData();
  };
  ...
  const exportExcel = () => {
    if (visits.length === 0) {
      toast.error("Nenhuma visita para exportar");
      return;
    }
  ...
  const pendingVisits = visits.filter((v) => v.status === "agendado").length;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          <Button onClick={exportExcel} variant="outline" disabled={visits.length === 0}>
            ...
            <DialogContent className="glass-panel border-border max-w-md">
              ...
              <form onSubmit={handleCreate} className="space-y-4">
                <div className="space-y-2">
                  <Label>Título</Label>
                  <Input value={form.titulo} onChange={(e) => setForm({ ...form, titulo: e.target.value })} placeholder="Ex: Instalação de equipamento" className="bg-secondary/50" required />
                </div>
                <div className="space-y-2">
                  <Label>Descrição</Label>
                  <Textarea value={form.descricao} onChange={(e) => setForm({ ...form, descricao: e.target.value })} placeholder="Detalhes da visita" className="bg-secondary/50" rows={2} />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-2">
                    <Label>Data</Label>
                    <Input type="date" value={form.visit_date} onChange={(e) => setForm({ ...form, visit_date: e.target.value })} className="bg-secondary/50" required />
                  </div>
                  <div className="space-y-2">
                    <Label>Hora</Label>
                    <Input type="time" value={form.visit_time} onChange={(e) => setForm({ ...form, visit_time: e.target.value })} className="bg-secondary/50" required />
                  </div>
                </div>
                <div className="space-y-2">
                  <Label>Técnico</Label>
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    {holiday && (
                      <span className="text-[8px] text-destructive font-medium truncate ml-1">🎉</span>
                    )}
                  </div>
                  {holiday && (
                    <p className="text-[8px] text-destructive/80 truncate leading-tight">{holiday.name}</p>
                  )}
                  <div className="mt-0.5 space-y-0.5">
                    ...
                    {dayV.length > 2 && (
                      <p className="text-[8px] text-muted-foreground">+{dayV.length - 2} mais</p>
                    )}
                  </div>
                </button>
              );
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1 text-xs text-muted-foreground">
                      ...
                      <span style={{ color: getMemberColor(v.member_id, v.cor) }}>
                        {getMemberName(v.member_id)}
                      </span>
                    </div>
                    {v.descricao && (
        ...
        <Card className="glass-panel neon-border">
          <CardContent className="p-4">
            ...
            <div className="flex flex-wrap gap-3">
              {members
                .filter((m) => visits.some((v) => v.member_id === m.id))
                .map((m) => {
                  const memberVisits = visits.filter((v) => v.member_id === m.id);
